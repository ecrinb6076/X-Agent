...
def run_oauth1_flow() -> tuple[str, str]:
    ...
    use_oob = is_truthy(os.getenv("X_OAUTH_USE_OOB", "0"))
    ...
    try:
        ...
        if "oob" not in callback_url and ("417" in str(e) or "Desktop" in str(e)):
            OAUTH_LOGGER.info("Callback rejected (desktop app). Falling back to PIN-based OOB flow.")
            callback_url = "oob"
            oauth = OAuth1Session(
                client_key=consumer_key,
                client_secret=consumer_secret,
    ...
    if callback_url == "oob":
        print("\n" + "=" * 50)
        print("Authorize the app in your browser, then enter the PIN below.")
        print("=" * 50)
        oauth_verifier = input("PIN: ").strip()
    else:
...
def _token_cache_path() -> Path:
    return Path(__file__).resolve().parent / ".oauth_tokens"


def _save_tokens(access_token: str, access_secret: str) -> None:
    path = _token_cache_path()
    path.write_text(f"{access_token}\n{access_secret}\n")
    path.chmod(0o600)
    OAUTH_LOGGER.info("OAuth1 tokens cached to %s", path)


def _load_cached_tokens() -> tuple[str, str] | None:
    ...
    if env_token and env_secret:
        OAUTH_LOGGER.info("Using OAuth1 tokens from environment.")
    ...
    if path.exists():
        ...
        if len(lines) >= 2 and lines[0] and lines[1]:
            OAUTH_LOGGER.info("Using cached OAuth1 tokens from %s", path)
...
def build_oauth1_client() -> OAuth1Client:
    ...
    if cached:
        access_token, access_secret = cached
    else:
